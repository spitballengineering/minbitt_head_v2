...
// Create a slider property with name `N`, flags `F` and the `X`, `Y`, `Z`, `W` coordinates


#define PSLIDER_NO_TIP(N, F, V, MIN, MAX) RAYGUI_CLITERAL(GuiDMProperty){N, GUI_PROP_SLIDER, F, .value.vslider = {V,MIN,MAX,NULL}}
#define PSLIDER_TIP(N, F, V, MIN, MAX, T) RAYGUI_CLITERAL(GuiDMProperty){N, GUI_PROP_SLIDER, F, .value.vslider = {V,MIN,MAX,T}}

// allows for PSLIDER overloading
// so both PSLIDER("x", 0, 0, 0, 100) and PSLIDER("x", 0, 0, 0, 100, "test") are valid
#define GET_6TH_ARG(N, F, V, MIN, MAX, arg6, arg7, ...) arg7
#define PSLIDER_MACRO_CHOOSER(...) \
    GET_6TH_ARG(__VA_ARGS__, PSLIDER_TIP, PSLIDER_NO_TIP, )

#define PSLIDER(...) PSLIDER_MACRO_CHOOSER(__VA_ARGS__)(__VA_ARGS__)
...
typedef struct {
    ...
    union {
        ...
        Rectangle vrect;
        Color vcolor;
        struct { float val; float min; float max; char *tooltip; } vslider;
    } value;
} GuiDMProperty;
...
double GuiDMSpinner(Rectangle bounds, double value, double minValue, double maxValue, double step, int precision, bool editMode) {
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        {
            if (GUI_BUTTON_PRESSED) state = STATE_PRESSED;
...
void GuiDMPropertyList(Rectangle bounds, GuiDMProperty* props, int count, int* focus, int* scrollIndex) {
    ...
    const int propSlots[] = {1,1,1,2,1,3,4,5,5,5,1,1};
        ...
        for(int p=0; p<count; ++p) 
        {
            ...
            if(absoluteBounds.y + currentHeight + height >= bounds.y && absoluteBounds.y + currentHeight <= bounds.y + bounds.height) 
            {
                ...
                switch(props[p].type) 
                {
                    ...
                    case GUI_PROP_SLIDER: {
                        #if !defined(RAYGUI_SLIDER_MAX_CHARS)
                            #define RAYGUI_SLIDER_MAX_CHARS  32
                        #endif
                        // draw property name
                        GuiDrawText(props[p].name, (Rectangle){propBounds.x + PROPERTY_PADDING, propBounds.y, propBounds.width/2-PROPERTY_PADDING, GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT)}, TEXT_ALIGN_LEFT, textColor);
                        // draw property value
                        Rectangle sliderRec = {propBounds.x+propBounds.width/2, propBounds.y + 1, propBounds.width/2, GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT) - 2};
                        GuiSlider(sliderRec, NULL, NULL, &props[p].value.vslider.val, props[p].value.vslider.min, props[p].value.vslider.max);

                        // copyied from GuiSlider because user might manualy move slider
                        char textLeft[RAYGUI_SLIDER_MAX_CHARS];
                        snprintf(textLeft, RAYGUI_SLIDER_MAX_CHARS + 1, "%.2f", props[p].value.vslider.val);

                        Rectangle textBounds = { 0 };
                        textBounds.width = (float)GuiGetTextWidth(textLeft);
                        textBounds.height = (float)GuiGetStyle(DEFAULT, TEXT_SIZE);
                        textBounds.x = sliderRec.x - textBounds.width - GuiGetStyle(SLIDER, TEXT_PADDING);
                        textBounds.y = sliderRec.y + sliderRec.height/2 - GuiGetStyle(DEFAULT, TEXT_SIZE)/2;

                        GuiDrawText(textLeft, textBounds, TEXT_ALIGN_RIGHT, GetColor(GuiGetStyle(LABEL, TEXT + (state*3))));

                        if (CheckCollisionPointRec(GUI_POINTER_POSITION, sliderRec)) GuiSetTooltip(props[p].value.vslider.tooltip);
                    }break;
