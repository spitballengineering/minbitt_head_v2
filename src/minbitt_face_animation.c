...
void minbitt_face_cartridge(blendshapes* blendshapes, float dt, controller_input *controller_input, const connection_t* connection, Image *frontScreenBuf){
    const int screenWidth = 1280;//TODO: pass in
    const int screenHeight = 720;
    // custom gifs
    if (controller_input->l_trigger) {
        unsigned int *key_combo = (unsigned int *) controller_input;
        switch (*key_combo) {
            case 0b000: DrawRectangle(screenWidth / 2, screenHeight / 2, 80, 80, RED);
                break;
            case 0b001: DrawRectangle(screenWidth / 2, screenHeight / 2, 80, 80, BLUE);
                break;
            case 0b111: DrawRectangle(screenWidth / 2, screenHeight / 2, 80, 80, GREEN);
                break;
            default: break; //TODO: show black
        }
    }

    if (!blendshapes->trackingStatus) DrawRectangleLinesEx((Rectangle){0,0, (float)screenWidth, (float)screenHeight}, 10, RED);
}
